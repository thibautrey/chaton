...
import type { ChatonsExtension, ChatonsExtensionQuickAction } from '@/features/workspace/types'
...
function getDecayedScore(row: QuickActionUsageRow, nowMs: number): number {
  const lastMs = row.last_used_at ? Date.parse(row.last_used_at) : Date.parse(row.updated_at)
...
function isValidQuickAction(value: unknown): value is ChatonsExtensionQuickAction {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false
  const action = value as Record<string, unknown>
  return typeof action.id === 'string' && action.id.length > 0 && typeof action.title === 'string' && action.title.length > 0
}

function getExtensionQuickActions(extension: ChatonsExtension): ChatonsExtensionQuickAction[] {
...
function shouldRenderCard(
  ...
  if (scope === 'global-or-no-thread') return selectedProjectId === null
    ? true
...
function emitExtensionDeeplink(viewId: string, target: string, params?: Record<string, unknown>) {
...
export function QuickActionCards() {
  ...
  const { createConversationGlobal, importProject, openExtensionMainView, state } = useWorkspace()
  const [extensions, setExtensions] = useState<ChatonsExtension[]>([])
  const [usageById, setUsageById] = useState<Record<string, QuickActionUsageRow>>({})

  useEffect(() => {
    let cancelled = false
    void Promise.all([workspaceIpc.listExtensions(), workspaceIpc.quickActionsListUsage()]).then(([extResult, usageResult]) => {
      if (cancelled) return
  ...
  const builtinCards: QuickCard[] = [
    ...
    },
    {
      ...
      onClick: () =>
        startConversationWithPrompt(
          [
            "Je veux créer une nouvelle compétence (skill) pour Codex/Chatons.",
            ...
            if (action.deeplink?.viewId && action.deeplink?.target) {
              if (action.deeplink.createConversation) {
                ...
                if (conversation && action.deeplink.prefillPrompt) {
                  emitComposerPrefill(conversation.id, action.deeplink.prefillPrompt)
                }
              }
              openExtensionMainView(action.deeplink.viewId)
              window.setTimeout(() => {
                emitExtensionDeeplink(action.deeplink!.viewId, action.deeplink!.target, action.deeplink!.params)
              }, 50)
              return
            }
  ...
  const cards = useMemo(() => {
    const all = [...builtinCards, ...extensionCards]
      .filter((card) => shouldRenderCard(card.scope, state.selectedProjectId, Boolean(state.selectedConversationId)))
  ...
  }, [builtinCards, extensionCards, state.selectedConversationId, state.selectedProjectId, usageById])
  ...
  return (
    <div className="quick-action-cards">
      <div className="quick-action-cards-track" role="list">
        {cards.map((card) => {
          const icon =
            card.icon === 'plus' ? <Plus size={28} /> :
            card.icon === 'workflow' ? <Workflow size={28} /> :
            card.icon === 'folder' ? <FolderPlus size={28} /> :
            <Sparkles size={28} />
          return (
            <button key={card.id} type="button" className="quick-action-card" onClick={() => void handleActionClick(card)} role="listitem">
              <div className="quick-action-card-icon">{icon}</div>
              <div className="quick-action-card-title">{card.title}</div>
              {card.description ? <div className="quick-action-card-description">{card.description}</div> : null}
            </button>
          )
        })}
      </div>
