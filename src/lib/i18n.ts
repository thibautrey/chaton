import i18n from "i18next";
import { initReactI18next } from "react-i18next";

i18n.use(initReactI18next).init({
  resources: {
    en: {
      translation: {
        // English translations will be added here
        "Redimensionner la barre latérale": "Resize sidebar",
        "Navigation principale": "Main navigation",
        Conversations: "Conversations",
        "Filtrer les conversations": "Filter conversations",
        "Rechercher une conversation": "Search for a conversation",
        "Liste chronologique": "Chronological list",
        Automatisations: "Automations",
        Compétences: "Skills",
        Extensions: "Extensions",
        Channels: "Channels",
        "Configurez vos passerelles de messagerie externes comme Telegram ou WhatsApp.":
          "Configure your external messaging bridges such as Telegram or WhatsApp.",
        "Aucun channel installé": "No channel installed",
        "Installez une extension de type channel pour connecter Chatons a Telegram, WhatsApp ou d'autres messageries externes.":
          "Install a channel extension to connect Chatons to Telegram, WhatsApp, or other external messaging systems.",
        Configurer: "Configure",
        Paramètres: "Settings",
        Général: "General",
        Audio: "Audio",
        Modèles: "Models",
        Providers: "Providers",
        "Packages & Extensions": "Packages & Extensions",
        "Outils & Exécution": "Tools & Execution",
        "Sessions & Export": "Sessions & Export",
        "Commandes Pi": "Pi Commands",
        Diagnostic: "Diagnostics",
        "Queue Pi": "Pi Queue",
        "Commandes en attente": "Pending commands",
        "Message utilisateur en attente": "Pending user message",
        "État runtime": "Runtime status",
        "Messages en attente (session Pi)": "Pending messages (Pi session)",
        "Gérer worktree": "Manage worktree",
        "Gestion du worktree": "Worktree management",
        Chargement: "Loading",
        "Chargement...": "Loading...",
        "Installation en cours...": "Installing...",
        "Installation...": "Installing...",
        "Installation annulée.": "Installation cancelled.",
        "Annuler l'installation": "Cancel installation",
        "Impossible d'annuler l'installation.":
          "Unable to cancel installation.",
        Branche: "Branch",
        Base: "Base",
        "Commits en avance": "Ahead commits",
        "Commits en retard": "Behind commits",
        "Déjà mergé dans la base": "Already merged into base",
        "Déjà pushé (upstream)": "Already pushed (upstream)",
        "Activer worktree": "Enable worktree",
        "Désactiver worktree": "Disable worktree",
        "Worktree activé.": "Worktree enabled.",
        "Worktree désactivé.": "Worktree disabled.",
        "Impossible de désactiver le worktree.": "Unable to disable worktree.",
        "Impossible de désactiver: modifications non commitées.":
          "Unable to disable: uncommitted changes exist.",
        "Impossible de lire le worktree": "Unable to read worktree",
        "Message de commit": "Commit message",
        "Écrire un message de commit...": "Write a commit message...",
        "Générer un message": "Generate message",
        "Génération...": "Generating...",
        "Commit...": "Committing...",
        Committer: "Commit",
        "Merge...": "Merging...",
        "Merger vers main": "Merge to main",
        "Push...": "Pushing...",
        Fermer: "Close",
        "Nouvelle conversation": "New conversation",
        Oui: "Yes",
        Non: "No",
        Queue: "Queue",
        "Voir plus": "See more",
        "Session dir": "Session directory",
        "Ouvrir dossier sessions": "Open sessions folder",
        "Export HTML (exemple)": "Export HTML (example)",
        "Ouvrir le dossier du projet": "Open project folder",
        "Ouvrir le terminal du projet": "Open project terminal",
        "Confirmer la suppression de {{title}}":
          "Confirm deletion of {{title}}",
        "Supprimer {{title}}": "Delete {{title}}",
        "Cliquer à nouveau pour supprimer": "Click again to delete",
        "Supprimer la conversation": "Delete conversation",
        "Confirmer la suppression de {{name}}": "Confirm deletion of {{name}}",
        "Supprimer {{name}}": "Delete {{name}}",
        "Supprimer le projet": "Delete project",
        Supprimer: "Delete",
        "Masquer les conversations": "Hide conversations",
        "Plus de conversations": "More conversations",
        Langue: "Language",
        "Sélectionnez la langue": "Select language",
        "Mise à jour disponible": "Update available",
        Téléchargement: "Downloading",
        "Mise à jour": "Update",
        Changelog: "Changelog",
        "Changelog pour la version": "Changelog for version",
        "Démarrez la conversation": "Start the conversation",
        "Écrivez votre premier message ci-dessous":
          "Write your first message below",
        "Créer une extension": "Create an extension",
        "Créer une nouvelle extension": "Create a new extension",
        "Créer une compétence": "Create a skill",
        "Créer une nouvelle compétence": "Create a new skill",
        "Ajouter un projet": "Add a project",
        "Aller en bas": "Go to bottom",
        "Chargement diagnostics...": "Loading diagnostics...",
        Rafraîchir: "Refresh",
        Comportements: "Behaviors",
        "Mémoire": "Memory",
        "Chargement des paramètres Pi...": "Loading Pi settings...",
        "Erreur: {{error}}": "Error: {{error}}",
        "Paramètres Pi": "Pi Settings",
        "Utilisation de la configuration Pi de l'utilisateur":
          "Using user's Pi configuration",
        "Utilisation de la configuration Pi locale embarquée":
          "Using embedded local Pi configuration",
        // Onboarding translations
        "onboarding.saving": "Saving...",
        "onboarding.continue": "Continue",
        "onboarding.selectModels":
          "Select which models should be available in Chatons.",
        "onboarding.loadingModels": "Loading models...",
        "onboarding.noModelsFound":
          "No model found for this provider yet. Add models in settings later.",
        "onboarding.runTest": "Run a quick test to verify your setup.",
        "onboarding.testing": "Testing...",
        "onboarding.openChatons": "Open Chatons",
        "onboarding.test.incompleteConfig": "Incomplete Pi configuration.",
        "onboarding.test.cannotLoadModels": "Unable to load models.",
        "onboarding.test.noModelsDetected":
          "No models detected. Check provider and models.",
        "onboarding.test.noModelsInScope": "No models selected in scope.",
        "onboarding.test.setupValidated":
          "Setup validated. Models are detected and scope is active.",
        "onboarding.error.cannotLoadModels": "Unable to load models.",
        "onboarding.error.cannotSaveScope": "Unable to save scope.",
        "Modèles disponibles": "Available Models",
        "{{count}} modèle(s) disponible(s)": "{{count}} model(s) available",
        Désactiver: "Disable",
        Activer: "Enable",
        "Modèles activés": "Enabled Models",
        "Aucun modèle activé": "No models enabled",
        "Paramètres supplémentaires": "Additional Settings",
        "Modèle par défaut": "Default Model",
        "Sélectionnez un modèle par défaut": "Select a default model",
        "Impossible de changer le statut de l'extension.":
          "Unable to change extension status.",
        "Health check exécuté pour {{name}}.":
          "Health check executed for {{name}}.",
        "Exécution de la commande en cours": "Command execution in progress",
        Exécuté: "Executed",
        pour: "for",
        fois: "times",
        "Impossible de supprimer cette extension.":
          "Unable to remove this extension.",
        "Installation impossible.": "Installation failed.",
        "{{name}} installée.": "{{name}} installed.",
        "Impossible d'ouvrir le dossier des extensions.":
          "Unable to open extensions folder.",
        "Ouvrir le dossier des extensions": "Open extensions folder",
        "📁 Ouvrir le dossier": "📁 Open folder",
        "Parcourez la bibliothèque d'extensions.":
          "Browse the extensions library.",
        "Parcourez la bibliothèque de compétences.":
          "Browse the skills library.",
        "Filtrer par nom, package ou description...":
          "Filter by name, package, or description...",
        Installées: "Installed",
        ID: "ID",
        Version: "Version",
        Health: "Health",
        "Sandbox: activée": "Sandbox: enabled",
        "Dernier run": "Last run",
        Réparer: "Repair",
        "Voir logs": "View logs",
        "Relancer Chatons": "Restart Chatons",
        "Extensions mises en avant": "Highlighted extensions",
        "Aucune extension trouvée avec le format npm @user/chatons-extension-name.":
          "No extension found matching npm format @user/chatons-extension-name.",
        Extension: "Extension",
        "Nécessite un redémarrage après installation.":
          "Requires restart after installation.",
        Installée: "Installed",
        Installer: "Install",
        Découvrir: "Discover",
        "Autres extensions": "Other extensions",
        "Updates Available": "Updates Available",
        "Mises à jour disponibles": "Updates Available",
        "Mettre à jour": "Update",
        "Mise à jour en cours...": "Updating...",
        "Mise à jour impossible.": "Update failed.",
        "{{name}} mise à jour.": "{{name}} updated.",
        "Tout mettre à jour": "Update All",
        "Mettre à jour la sélection": "Update Selected",
        "Aucune mise à jour disponible.": "No updates available.",
        "{{count}} extensions mises à jour.": "{{count}} extensions updated.",
        "Échec de la mise à jour des extensions.":
          "Failed to update extensions.",
        "Aucune extension sélectionnée pour la mise à jour.":
          "No extensions selected for update.",
        Publier: "Publish",
        "Publier sur npm": "Publish to npm",
        "Publication en cours...": "Publishing...",
        "Impossible de publier cette extension.":
          "Unable to publish this extension.",
        "{{name}} publiée.": "{{name}} published.",
        "npm Login Required": "npm Login Required",
        "You need to be logged in to npm to publish extensions. Please provide your npm token.":
          "You need to be logged in to npm to publish extensions. Please provide your npm token.",
        "Get your token from:": "Get your token from:",
        "npm tokens page": "npm tokens page",
        "npm Token": "npm Token",
        "(replace {{your-username}} with your actual npm username)":
          "(remplacez {{your-username}} par votre nom d'utilisateur npm)",
        "Enter your npm token": "Enter your npm token",
        "Publish with Token": "Publish with Token",
        Cancel: "Cancel",
        "Update Available": "Update Available",
        Sélectionner: "Select",
        Source: "Source",
        builtin: "builtin",
        npm: "npm",
        "Impossible de lister les compétences Pi.": "Unable to list Pi skills.",
        "Aucune compétence installée.": "No skill installed.",
        "Compétences mises en avant": "Highlighted skills",
        "Aucune compétence disponible à découvrir.":
          "No skill available to discover.",
        Compétence: "Skill",
        Chemin: "Path",
        Désinstaller: "Uninstall",
        "{{name}} désinstallée.": "{{name}} uninstalled.",
        Auteur: "Author",
        "Conversation terminée": "Conversation completed",
        'La conversation "{{title}}" a terminé son action':
          'The conversation "{{title}}" has completed its action',
        "Sélecteur de mode d'accès agent": "Agent access mode selector",
        "Passer en mode sécurisé": "Switch to secure mode",
        "Passer en mode ouvert": "Switch to open mode",
        "Mode sécurisé: comportement actuel, accès limité au contexte de la conversation.":
          "Secure mode: current behavior, access limited to the conversation context.",
        "Mode ouvert: Chaton peut accéder à des fichiers/dossiers hors contexte initial et exécuter les commandes nécessaires.":
          "Open mode: Chaton can access files/folders outside the initial context and run required commands.",
        sécurisé: "secure",
        ouvert: "open",
        "Quel mode choisir ?": "Which mode should I choose?",
        "Reste centré sur cette conversation.":
          "Stays focused on this conversation.",
        "Le plus prudent pour la plupart des échanges.":
          "Best default for most chats.",
        "Peut aller plus loin pour réaliser des tâches complètes.":
          "Can go further to complete broader tasks.",
        "Pratique quand vous voulez une aide plus large.":
          "Useful when you want wider help.",
        "notice.sendMessage.failed": "Unable to send message to the model.",
        "notice.sendMessage.authFailed":
          "Unable to send message: authentication failed (401/access denied). Check the provider API key.",
        "notice.sendMessage.failedWithReason":
          "Unable to send message to the model: {{reason}}",
        "composer.accessModeChangeFailed": "Unable to change agent access mode.",
        "Chime à la fin des conversations": "Chime at end of conversations",
        "Memory injected": "Memory injected",
        "Saving to memory...": "Saving to memory...",
        "Saved to memory": "Saved to memory",
        // Assistant mode
        Workspace: "Workspace",
        Assistant: "Assistant",
        "Mode de l'application": "Application mode",
        "Navigation assistant": "Assistant navigation",
        "Votre assistant personnel": "Your personal assistant",
        "Bienvenue, {{name}}": "Welcome, {{name}}",
        "Toujours disponible, connecte a vos messageries, et apprend de vos preferences au fil du temps.":
          "Always available, connected to your messaging apps, and learns your preferences over time.",
        "Configurer mon assistant": "Set up my assistant",
        // Assistant onboarding
        "assistant.onboarding.welcomeTitle": "Meet your personal assistant",
        "assistant.onboarding.welcomeSubtitle": "Always available, connected to your favorite messaging apps, and continuously learning your preferences.",
        "assistant.onboarding.getStarted": "Get started",
        "assistant.onboarding.stepChannel": "Connect",
        "assistant.onboarding.stepPersonalize": "Personalize",
        "assistant.onboarding.stepDone": "Done",
        "assistant.onboarding.channelTitle": "Connect a messaging app",
        "assistant.onboarding.channelDesc": "Choose how you want to reach your assistant. You can connect Telegram, WhatsApp, or other messaging apps.",
        "assistant.onboarding.installedChannels": "Ready to use",
        "assistant.onboarding.availableChannels": "Available to install",
        "assistant.onboarding.noChannelsAvailable": "No messaging integrations are available yet. You can add them later from the extensions library.",
        "assistant.onboarding.skipStep": "Skip for now",
        "assistant.onboarding.back": "Back",
        "assistant.onboarding.personalizeTitle": "Make it yours",
        "assistant.onboarding.personalizeDesc": "Give your assistant a name and tell it who you are.",
        "assistant.onboarding.assistantNameLabel": "Assistant name",
        "assistant.onboarding.userNameLabel": "Your name",
        "assistant.onboarding.userNamePlaceholder": "How should it call you?",
        "assistant.onboarding.completeTitle": "{{name}} is ready!",
        "assistant.onboarding.completeDesc": "Your assistant is set up and ready to help. You can always change these settings later.",
        "assistant.onboarding.openAssistant": "Open my assistant",

        "assistant.greeting.morning": "Good morning",
        "assistant.greeting.afternoon": "Good afternoon",
        "assistant.greeting.evening": "Good evening",
        "assistant.greeting.night": "Good evening",

        "assistant.nav.home": "Home",
        "assistant.nav.conversations": "Conversations",
        "assistant.nav.memory": "Memory",
        "assistant.nav.automations": "Automations",
        "assistant.nav.channels": "Messaging",

        "assistant.dashboard.online": "{{name}} is online",
        "assistant.dashboard.offline": "{{name}} is offline",
        "assistant.dashboard.channels": "Your messaging apps",
        "assistant.dashboard.loading": "Loading...",
        "assistant.dashboard.noChannels": "No messaging app connected yet.",
        "assistant.dashboard.addChannel": "Add a messaging app",
        "assistant.dashboard.connected": "Connected",
        "assistant.dashboard.notConfigured": "Not configured",
        "assistant.dashboard.quickActions": "What would you like to do?",
        "assistant.dashboard.talkTo": "Chat",
        "assistant.dashboard.scheduleTask": "Schedule a task",
        "assistant.dashboard.viewMemory": "What I know",
        "assistant.dashboard.manageSettings": "Settings",
        "assistant.dashboard.recentActivity": "Recent activity",
        "assistant.dashboard.noActivity": "No conversations yet. Start one to see it here!",
        "assistant.dashboard.untitled": "Untitled conversation",
        "assistant.dashboard.viaChannel": "via messaging",
        "assistant.dashboard.justNow": "Just now",
        "assistant.dashboard.minutesAgo": "{{count}} min ago",
        "assistant.dashboard.hoursAgo": "{{count}}h ago",
        "assistant.dashboard.yesterday": "Yesterday",
        "assistant.dashboard.daysAgo": "{{count}} days ago",
        "assistant.dashboard.memory": "What I remember",
        "assistant.dashboard.noMemories": "I haven't memorized anything yet.",
        "assistant.dashboard.memoryHint": "I learn your preferences as we chat together.",
        "assistant.dashboard.viewAll": "View all",
        "assistant.dashboard.automations": "Your automations",
        "assistant.dashboard.noAutomations": "No automation set up yet.",
        "assistant.dashboard.createAutomation": "Create an automation",
        "assistant.dashboard.manageAutomations": "Manage automations",

        "assistant.memory.title": "Memory",
        "assistant.memory.desc": "Everything your assistant has remembered about you and your preferences.",
        "assistant.memory.empty": "Nothing memorized yet",
        "assistant.memory.emptyHint": "Your assistant learns as you interact with it. Preferences, context, and useful information are saved here automatically.",
        "assistant.memory.delete": "Delete",
        "assistant.memory.settings": "Memory settings",
        "assistant.memory.modelLabel": "Memory model",
        "assistant.memory.modelHint": "Choose a specific model for summarizing and managing memory, or leave on auto to use the last model used in conversation.",
        "assistant.memory.modelAuto": "Auto (last used model)",

        "settings.memory.title": "Memory",
        "settings.memory.desc": "Configure how Chatons remembers and uses context from past conversations.",
        "settings.memory.modelLabel": "Memory model",
        "settings.memory.modelHint": "Choose which model is used to summarize conversations and manage memory. Auto uses the last model from the conversation.",
        "settings.memory.modelAuto": "Auto (last used model)",
        "settings.memory.saved": "Saved!",

        "settings.autocomplete.title": "Auto-complete",
        "settings.autocomplete.desc": "Configure AI-powered autocomplete suggestions in the composer. Suggestions appear as you type and can be accepted with Tab.",
        "settings.autocomplete.enabled": "Auto-complete enabled",
        "settings.autocomplete.disabled": "Auto-complete disabled",
        "settings.autocomplete.modelLabel": "Completion model",
        "settings.autocomplete.modelAuto": "Auto (last used model)",
        "settings.autocomplete.hint": "Suggestions are generated as you type. Press Tab to accept or keep typing to dismiss.",

        "settings.title.title": "Titles",
        "settings.title.desc": "Configure how Chatons generates conversation titles.",
        "settings.title.modelLabel": "Title generation model",
        "settings.title.modelHint": "Choose which model is used to generate conversation titles. If not set, the conversation's current model will be used.",
        "settings.title.selectModel": "Select a model",

        "assistant.automations.title": "Automations",
        "assistant.automations.desc": "Tasks that your assistant runs automatically for you.",
        "assistant.automations.empty": "No automation yet",
        "assistant.automations.emptyHint": "Set up recurring tasks or triggered actions to save time.",
        "assistant.automations.trigger": "Trigger",
        "assistant.automations.manage": "Manage in automation panel",

        "assistant.channels.title": "Messaging apps",
        "assistant.channels.desc": "Connect your favorite apps to chat with your assistant from anywhere.",
        "assistant.channels.empty": "No messaging app installed yet.",
        "assistant.channels.install": "Browse available apps",
        "assistant.channels.configure": "Configure",
        "assistant.channels.addMore": "Add more apps",

        "assistant.sheet.close": "Close",
        "assistant.sheet.viewNotFound": "This view could not be found.",
        "assistant.sheet.loadError": "Unable to load the configuration view.",
        // Tool Call Display Mode
        "Verbose": "Verbose",
        "Light": "Light",
        "Quiet": "Quiet",
        "Affichage des appels d'outils": "Tool call display",
        "toolCallDisplayMode.verbose.description": "Full display: tool traces expanded and shown",
        "toolCallDisplayMode.light.description": "Compact: tool names shown but traces collapsed",
        "toolCallDisplayMode.quiet.description": "Hidden: no tool calls shown in conversations",
      },
    },
    fr: {
      translation: {
        "Redimensionner la barre latérale": "Redimensionner la barre latérale",
        Langue: "Langue",
        "Ajouter package": "Ajouter package",
        "Remplacer la valeur": "Replace value",
        "Aucune commande exécutée.": "No command executed.",
        Succès: "Success",
        Erreur: "Error",
        "Ajouter provider": "Add provider",
        "Ajouter un provider": "Add a provider",
        "ex: openai-codex": "e.g., openai-codex",
        Annuler: "Cancel",
        "{{scopedCount}} sur {{total}} dans le scope":
          "{{scopedCount}} out of {{total}} in scope",
        "Déplier provider": "Expand provider",
        "Replier provider": "Collapse provider",
        "Supprimer provider": "Delete provider",
        "Aucun modèle détecté via `pi --list-models`.": "No models detected",
        "Ajouter un nouveau projet": "Add a new project",
        "Aucune conversation pour ce projet": "No conversations for this project",
        "Aucun log trouvé": "No logs found",
        "Aucune modification à commit.": "No changes to commit.",
        "Message de commit requis.": "Commit message required.",
        "Impossible de générer un message de commit.":
          "Unable to generate a commit message.",
        "Commit impossible.": "Unable to commit.",
        "Aucune extension installée.": "No extensions installed.",
        "Par projet": "By project",
        "Liste chronologique": "Chronological list",
        Créé: "Created",
        "Mis à jour": "Updated",
        "Toutes les conversations": "All conversations",
        Pertinente: "Relevant",
        Organiser: "Organize",
        "Trier par": "Sort by",
        Afficher: "Show",
        "Afficher les stats assistant": "Show assistant stats",
        "Filtrer trier et organiser les conversations":
          "Filter, sort and organize conversations",
        "Masquer les modifications": "Hide modifications",
        "Afficher les modifications": "Show modifications",
        "fichiers modifies": "files modified",
        "fichier modifie": "file modified",
        "Sélectionnez la langue": "Sélectionnez la langue",
        "Navigation principale": "Navigation principale",
        Conversations: "Conversations",
        "Filtrer les conversations": "Filtrer les conversations",
        "Rechercher une conversation": "Rechercher une conversation",
        Automatisations: "Automatisations",
        Compétences: "Compétences",
        Extensions: "Extensions",
        Channels: "Channels",
        "Configurez vos passerelles de messagerie externes comme Telegram ou WhatsApp.":
          "Configurez vos passerelles de messagerie externes comme Telegram ou WhatsApp.",
        "Aucun channel installé": "Aucun channel installé",
        "Installez une extension de type channel pour connecter Chatons a Telegram, WhatsApp ou d'autres messageries externes.":
          "Installez une extension de type channel pour connecter Chatons a Telegram, WhatsApp ou d'autres messageries externes.",
        Configurer: "Configurer",
        Paramètres: "Paramètres",
        Général: "Général",
        Modèles: "Modèles",
        Providers: "Providers",
        "Packages & Extensions": "Packages & Extensions",
        "Outils & Exécution": "Outils & Exécution",
        "Sessions & Export": "Sessions & Export",
        "Commandes Pi": "Commandes Pi",
        Diagnostic: "Diagnostic",
        "Queue Pi": "Queue Pi",
        "Commandes en attente": "Commandes en attente",
        "Message utilisateur en attente": "Message utilisateur en attente",
        "État runtime": "État runtime",
        "Messages en attente (session Pi)": "Messages en attente (session Pi)",
        "notice.sendMessage.failed":
          "Impossible d'envoyer le message au modèle.",
        "notice.sendMessage.authFailed":
          "Impossible d'envoyer le message: échec d'authentification (401/accès refusé). Vérifiez la clé API du provider.",
        "notice.sendMessage.failedWithReason":
          "Impossible d'envoyer le message au modèle: {{reason}}",
        "composer.accessModeChangeFailed": "Impossible de changer le mode d'accès de l'agent.",
        "Gérer worktree": "Gérer worktree",
        "Gestion du worktree": "Gestion du worktree",
        Chargement: "Chargement",
        "Chargement...": "Chargement...",
        Branche: "Branche",
        Base: "Base",
        "Commits en avance": "Commits en avance",
        "Commits en retard": "Commits en retard",
        "Déjà mergé dans la base": "Déjà mergé dans la base",
        "Déjà pushé (upstream)": "Déjà pushé (upstream)",
        "Activer worktree": "Activer worktree",
        "Désactiver worktree": "Désactiver worktree",
        "Worktree activé.": "Worktree activé.",
        "Worktree désactivé.": "Worktree désactivé.",
        "Impossible de désactiver le worktree.":
          "Impossible de désactiver le worktree.",
        "Projet introuvable.": "Projet introuvable.",
        "Impossible d'ouvrir le dossier du projet.":
          "Impossible d'ouvrir le dossier du projet.",
        "Impossible de désactiver: modifications non commitées.":
          "Impossible de désactiver: modifications non commitées.",
        "Impossible de lire le worktree": "Impossible de lire le worktree",
        "Message de commit": "Message de commit",
        "Écrire un message de commit...": "Écrire un message de commit...",
        "Générer un message": "Générer un message",
        "Génération...": "Génération...",
        "Commit...": "Commit...",
        Committer: "Committer",
        "Merge...": "Merge...",
        "Merger vers main": "Merger vers main",
        "Push...": "Push...",
        Fermer: "Fermer",
        "Nouvelle conversation": "Nouvelle conversation",
        Oui: "Oui",
        Non: "Non",
        Queue: "Queue",
        "Voir plus": "Voir plus",
        "Session dir": "Session dir",
        "Ouvrir dossier sessions": "Ouvrir dossier sessions",
        "Export HTML (exemple)": "Export HTML (exemple)",
        "Ouvrir le dossier du projet": "Ouvrir le dossier du projet",
        "Ouvrir le terminal du projet": "Ouvrir le terminal du projet",
        "Confirmer la suppression de {{title}}":
          "Confirmer la suppression de {{title}}",
        "Supprimer {{title}}": "Supprimer {{title}}",
        "Cliquer à nouveau pour supprimer": "Cliquer à nouveau pour supprimer",
        "Supprimer la conversation": "Supprimer la conversation",
        "Confirmer la suppression de {{name}}":
          "Confirmer la suppression de {{name}}",
        "Supprimer {{name}}": "Supprimer {{name}}",
        "Supprimer le projet": "Supprimer le projet",
        Supprimer: "Supprimer",
        "Mise à jour disponible": "Mise à jour disponible",
        Téléchargement: "Téléchargement",
        "Mise à jour": "Mise à jour",
        Changelog: "Changelog",
        "Changelog pour la version": "Changelog pour la version",
        "Démarrez la conversation": "Démarrez la conversation",
        "Écrivez votre premier message ci-dessous":
          "Écrivez votre premier message ci-dessous",
        "Créer une extension": "Créer une extension",
        "Créer une nouvelle extension": "Créer une nouvelle extension",
        "Créer une compétence": "Créer une compétence",
        "Créer une nouvelle compétence": "Créer une nouvelle compétence",
        "Ajouter un projet": "Ajouter un projet",
        "Aller en bas": "Aller en bas",
        "Chargement diagnostics...": "Chargement diagnostics...",
        Rafraîchir: "Rafraîchir",
        Comportements: "Comportements",
        "Mémoire": "Mémoire",
        "Chargement des paramètres Pi...": "Chargement des paramètres Pi...",
        "Erreur: {{error}}": "Erreur: {{error}}",
        "Paramètres Pi": "Paramètres Pi",
        "Utilisation de la configuration Pi de l'utilisateur":
          "Utilisation de la configuration Pi de l'utilisateur",
        "Utilisation de la configuration Pi locale embarquée":
          "Utilisation de la configuration Pi locale embarquée",
        // Onboarding translations
        "onboarding.saving": "Sauvegarde...",
        "onboarding.continue": "Continuer",
        "onboarding.selectModels":
          "Sélectionnez les modèles qui doivent être disponibles dans Chatons.",
        "onboarding.loadingModels": "Chargement des modèles...",
        "onboarding.noModelsFound":
          "Aucun modèle trouvé pour ce fournisseur. Ajoutez des modèles dans les paramètres plus tard.",
        "onboarding.runTest":
          "Exécutez un test rapide pour vérifier votre configuration.",
        "onboarding.testing": "Test en cours...",
        "onboarding.openChatons": "Ouvrir Chatons",
        "onboarding.test.incompleteConfig": "Configuration Pi incomplète.",
        "onboarding.test.cannotLoadModels":
          "Impossible de charger les modèles.",
        "onboarding.test.noModelsDetected":
          "Aucun modèle détecté. Vérifiez le fournisseur et les modèles.",
        "onboarding.test.noModelsInScope":
          "Aucun modèle sélectionné dans le scope.",
        "onboarding.test.setupValidated":
          "Setup validé. Les modèles sont détectés et le scope est actif.",
        "onboarding.error.cannotLoadModels":
          "Impossible de charger les modèles.",
        "onboarding.error.cannotSaveScope":
          "Impossible de sauvegarder le scope.",
        "Modèles disponibles": "Modèles disponibles",
        "{{count}} modèle(s) disponible(s)":
          "{{count}} modèle(s) disponible(s)",
        Désactiver: "Désactiver",
        Activer: "Activer",
        "Modèles activés": "Modèles activés",
        "Aucun modèle activé": "Aucun modèle activé",
        "Paramètres supplémentaires": "Paramètres supplémentaires",
        "Modèle par défaut": "Modèle par défaut",
        "Sélectionnez un modèle par défaut":
          "Sélectionnez un modèle par défaut",
        "Impossible de changer le statut de l'extension.":
          "Impossible de changer le statut de l'extension.",
        "Health check exécuté pour {{name}}.":
          "Health check exécuté pour {{name}}.",
        "Exécution de la commande en cours":
          "Exécution de la commande en cours",
        Exécuté: "Exécuté",
        pour: "pour",
        fois: "fois",
        "Impossible de supprimer cette extension.":
          "Impossible de supprimer cette extension.",
        "Installation impossible.": "Installation impossible.",
        "{{name}} installée.": "{{name}} installée.",
        "Impossible d'ouvrir le dossier des extensions.":
          "Impossible d'ouvrir le dossier des extensions.",
        "Ouvrir le dossier des extensions": "Ouvrir le dossier des extensions",
        "📁 Ouvrir le dossier": "📁 Ouvrir le dossier",
        "Parcourez la bibliothèque d'extensions.":
          "Parcourez la bibliothèque d'extensions.",
        "Parcourez la bibliothèque de compétences.":
          "Parcourez la bibliothèque de compétences.",
        "Filtrer par nom, package ou description...":
          "Filtrer par nom, package ou description...",
        Installées: "Installées",
        ID: "ID",
        Version: "Version",
        Health: "Health",
        "Sandbox: activée": "Sandbox: activée",
        "Dernier run": "Dernier run",
        Réparer: "Réparer",
        "Voir logs": "Voir logs",
        "Relancer Chatons": "Relancer Chatons",
        "Extensions mises en avant": "Extensions mises en avant",
        "Aucune extension trouvée avec le format npm @user/chatons-extension-name.":
          "Aucune extension trouvée avec le format npm @user/chatons-extension-name.",
        Extension: "Extension",
        "Nécessite un redémarrage après installation.":
          "Nécessite un redémarrage après installation.",
        Installée: "Installée",
        Installer: "Installer",
        Découvrir: "Découvrir",
        "Autres extensions": "Autres extensions",
        "Updates Available": "Mises à jour disponibles",
        "Mises à jour disponibles": "Mises à jour disponibles",
        "Mettre à jour": "Mettre à jour",
        "Mise à jour en cours...": "Mise à jour en cours...",
        "Mise à jour impossible.": "Mise à jour impossible.",
        "{{name}} mise à jour.": "{{name}} mise à jour.",
        "Tout mettre à jour": "Tout mettre à jour",
        "Mettre à jour la sélection": "Mettre à jour la sélection",
        "Aucune mise à jour disponible.": "Aucune mise à jour disponible.",
        "{{count}} extensions mises à jour.":
          "{{count}} extensions mises à jour.",
        "Échec de la mise à jour des extensions.":
          "Échec de la mise à jour des extensions.",
        "Aucune extension sélectionnée pour la mise à jour.":
          "Aucune extension sélectionnée pour la mise à jour.",
        Publier: "Publier",
        "Publier sur npm": "Publier sur npm",
        "Publication en cours...": "Publication en cours...",
        "Impossible de publier cette extension.":
          "Impossible de publier cette extension.",
        "{{name}} publiée.": "{{name}} publiée.",
        "npm Login Required": "Connexion npm requise",
        "You need to be logged in to npm to publish extensions. Please provide your npm token.":
          "Vous devez être connecté à npm pour publier des extensions. Veuillez fournir votre jeton npm.",
        "Get your token from:": "Obtenez votre jeton depuis:",
        "npm tokens page": "page des jetons npm",
        "npm Token": "Jeton npm",
        "(replace {{your-username}} with your actual npm username)":
          "(remplacez {{your-username}} par votre nom d'utilisateur npm)",
        "Enter your npm token": "Entrez votre jeton npm",
        "Publish with Token": "Publier avec le jeton",
        Cancel: "Annuler",
        "Update Available": "Mise à jour disponible",
        Sélectionner: "Sélectionner",
        Source: "Source",
        builtin: "builtin",
        npm: "npm",
        "Impossible de lister les compétences Pi.":
          "Impossible de lister les compétences Pi.",
        "Aucune compétence installée.": "Aucune compétence installée.",
        "Compétences mises en avant": "Compétences mises en avant",
        "Aucune compétence disponible à découvrir.":
          "Aucune compétence disponible à découvrir.",
        Compétence: "Compétence",
        Chemin: "Chemin",
        Désinstaller: "Désinstaller",
        "{{name}} désinstallée.": "{{name}} désinstallée.",
        Auteur: "Auteur",
        Sauvegarder: "Save",
        Appliquer: "Apply",
        "Prompt appliqué automatiquement au début de chaque message utilisateur.":
          "Prompt automatically applied at the beginning of each user message.",
        "Conversation terminée": "Conversation terminée",
        'La conversation "{{title}}" a terminé son action':
          'La conversation "{{title}}" a terminé son action',
        "Sélecteur de mode d'accès agent": "Sélecteur de mode d'accès agent",
        "Passer en mode sécurisé": "Passer en mode sécurisé",
        "Passer en mode ouvert": "Passer en mode ouvert",
        "Mode sécurisé: comportement actuel, accès limité au contexte de la conversation.":
          "Mode sécurisé: comportement actuel, accès limité au contexte de la conversation.",
        "Mode ouvert: Chaton peut accéder à des fichiers/dossiers hors contexte initial et exécuter les commandes nécessaires.":
          "Mode ouvert: Chaton peut accéder à des fichiers/dossiers hors contexte initial et exécuter les commandes nécessaires.",
        sécurisé: "sécurisé",
        ouvert: "ouvert",
        "Quel mode choisir ?": "Quel mode choisir ?",
        "Reste centré sur cette conversation.":
          "Reste centré sur cette conversation.",
        "Le plus prudent pour la plupart des échanges.":
          "Le plus prudent pour la plupart des échanges.",
        "Peut aller plus loin pour réaliser des tâches complètes.":
          "Peut aller plus loin pour réaliser des tâches complètes.",
        "Pratique quand vous voulez une aide plus large.":
          "Pratique quand vous voulez une aide plus large.",
        "Chime à la fin des conversations": "Chime à la fin des conversations",
        "Memory injected": "Mémoire injectée",
        "Saving to memory...": "Enregistrement en mémoire...",
        "Saved to memory": "Mémoire enregistrée",
        // Assistant mode
        Workspace: "Workspace",
        Assistant: "Assistant",
        "Mode de l'application": "Mode de l'application",
        "Navigation assistant": "Navigation assistant",
        "Votre assistant personnel": "Votre assistant personnel",
        "Bienvenue, {{name}}": "Bienvenue, {{name}}",
        "Toujours disponible, connecte a vos messageries, et apprend de vos preferences au fil du temps.":
          "Toujours disponible, connecte a vos messageries, et apprend de vos preferences au fil du temps.",
        "Configurer mon assistant": "Configurer mon assistant",
        // Assistant onboarding
        "assistant.onboarding.welcomeTitle": "Votre assistant personnel",
        "assistant.onboarding.welcomeSubtitle": "Toujours disponible, connecte a vos messageries preferees, et apprend de vos preferences au fil du temps.",
        "assistant.onboarding.getStarted": "Commencer",
        "assistant.onboarding.stepChannel": "Connexion",
        "assistant.onboarding.stepPersonalize": "Personnaliser",
        "assistant.onboarding.stepDone": "Termine",
        "assistant.onboarding.channelTitle": "Connecter une messagerie",
        "assistant.onboarding.channelDesc": "Choisissez comment contacter votre assistant. Vous pouvez connecter Telegram, WhatsApp ou d'autres messageries.",
        "assistant.onboarding.installedChannels": "Prets a utiliser",
        "assistant.onboarding.availableChannels": "Disponibles a installer",
        "assistant.onboarding.noChannelsAvailable": "Aucune integration de messagerie disponible pour le moment. Vous pourrez en ajouter plus tard depuis la bibliotheque d'extensions.",
        "assistant.onboarding.skipStep": "Passer cette etape",
        "assistant.onboarding.back": "Retour",
        "assistant.onboarding.personalizeTitle": "Personnalisez-le",
        "assistant.onboarding.personalizeDesc": "Donnez un nom a votre assistant et dites-lui qui vous etes.",
        "assistant.onboarding.assistantNameLabel": "Nom de l'assistant",
        "assistant.onboarding.userNameLabel": "Votre prenom",
        "assistant.onboarding.userNamePlaceholder": "Comment doit-il vous appeler ?",
        "assistant.onboarding.completeTitle": "{{name}} est pret !",
        "assistant.onboarding.completeDesc": "Votre assistant est configure et pret a vous aider. Vous pouvez modifier ces reglages a tout moment.",
        "assistant.onboarding.openAssistant": "Ouvrir mon assistant",

        "assistant.greeting.morning": "Bonjour",
        "assistant.greeting.afternoon": "Bon apres-midi",
        "assistant.greeting.evening": "Bonsoir",
        "assistant.greeting.night": "Bonsoir",

        "assistant.nav.home": "Accueil",
        "assistant.nav.conversations": "Conversations",
        "assistant.nav.memory": "Memoire",
        "assistant.nav.automations": "Automatisations",
        "assistant.nav.channels": "Messageries",

        "assistant.dashboard.online": "{{name}} est en ligne",
        "assistant.dashboard.offline": "{{name}} est hors ligne",
        "assistant.dashboard.channels": "Vos messageries",
        "assistant.dashboard.loading": "Chargement...",
        "assistant.dashboard.noChannels": "Aucune messagerie connectee.",
        "assistant.dashboard.addChannel": "Ajouter une messagerie",
        "assistant.dashboard.connected": "Connecte",
        "assistant.dashboard.notConfigured": "Non configure",
        "assistant.dashboard.quickActions": "Que souhaitez-vous faire ?",
        "assistant.dashboard.talkTo": "Discuter",
        "assistant.dashboard.scheduleTask": "Programmer une tache",
        "assistant.dashboard.viewMemory": "Ce que je sais",
        "assistant.dashboard.manageSettings": "Reglages",
        "assistant.dashboard.recentActivity": "Activite recente",
        "assistant.dashboard.noActivity": "Aucune conversation. Lancez-en une pour la voir ici !",
        "assistant.dashboard.untitled": "Conversation sans titre",
        "assistant.dashboard.viaChannel": "via messagerie",
        "assistant.dashboard.justNow": "A l'instant",
        "assistant.dashboard.minutesAgo": "Il y a {{count}} min",
        "assistant.dashboard.hoursAgo": "Il y a {{count}}h",
        "assistant.dashboard.yesterday": "Hier",
        "assistant.dashboard.daysAgo": "Il y a {{count}} jours",
        "assistant.dashboard.memory": "Ce que je retiens",
        "assistant.dashboard.noMemories": "Je n'ai encore rien memorise.",
        "assistant.dashboard.memoryHint": "J'apprends vos preferences au fil de nos echanges.",
        "assistant.dashboard.viewAll": "Tout voir",
        "assistant.dashboard.automations": "Vos automatisations",
        "assistant.dashboard.noAutomations": "Aucune automatisation configuree.",
        "assistant.dashboard.createAutomation": "Creer une automatisation",
        "assistant.dashboard.manageAutomations": "Gerer les automatisations",

        "assistant.memory.title": "Memoire",
        "assistant.memory.desc": "Tout ce que votre assistant a retenu sur vous et vos preferences.",
        "assistant.memory.empty": "Rien de memorise pour le moment",
        "assistant.memory.emptyHint": "Votre assistant apprend au fil de vos echanges. Preferences, contexte et informations utiles sont enregistres ici automatiquement.",
        "assistant.memory.delete": "Supprimer",
        "assistant.memory.settings": "Parametres de la memoire",
        "assistant.memory.modelLabel": "Modele de memoire",
        "assistant.memory.modelHint": "Choisissez un modele specifique pour resumer et gerer la memoire, ou laissez en auto pour utiliser le dernier modele utilise.",
        "assistant.memory.modelAuto": "Auto (dernier modele utilise)",

        "settings.memory.title": "Memoire",
        "settings.memory.desc": "Configurez comment Chatons memorise et utilise le contexte des conversations passees.",
        "settings.memory.modelLabel": "Modele de memoire",
        "settings.memory.modelHint": "Choisissez le modele utilise pour resumer les conversations et gerer la memoire. Auto utilise le dernier modele de la conversation.",
        "settings.memory.modelAuto": "Auto (dernier modele utilise)",
        "settings.memory.saved": "Enregistre !",

        "settings.autocomplete.title": "Auto-completion",
        "settings.autocomplete.desc": "Configurez les suggestions d'auto-completion IA dans le composeur. Les suggestions apparaissent au fur et a mesure de la frappe et peuvent etre acceptees avec Tab.",
        "settings.autocomplete.enabled": "Auto-completion activee",
        "settings.autocomplete.disabled": "Auto-completion desactivee",
        "settings.autocomplete.modelLabel": "Modele de completion",
        "settings.autocomplete.modelAuto": "Auto (dernier modele utilise)",
        "settings.autocomplete.hint": "Les suggestions sont generees au fur et a mesure de la frappe. Appuyez sur Tab pour accepter ou continuez a ecrire pour rejeter.",

        "settings.title.title": "Titres",
        "settings.title.desc": "Configurez comment Chatons genere les titres des conversations.",
        "settings.title.modelLabel": "Modele de generation des titres",
        "settings.title.modelHint": "Choisissez le modele utilise pour generer les titres des conversations. Si non defini, le modele actuel de la conversation sera utilise.",
        "settings.title.selectModel": "Selectionner un modele",

        "assistant.automations.title": "Automatisations",
        "assistant.automations.desc": "Les taches que votre assistant execute automatiquement pour vous.",
        "assistant.automations.empty": "Aucune automatisation",
        "assistant.automations.emptyHint": "Configurez des taches recurrentes ou des actions declenchees pour gagner du temps.",
        "assistant.automations.trigger": "Declencheur",
        "assistant.automations.manage": "Gerer dans le panneau automatisations",

        "assistant.channels.title": "Messageries",
        "assistant.channels.desc": "Connectez vos applications preferees pour discuter avec votre assistant de n'importe ou.",
        "assistant.channels.empty": "Aucune messagerie installee.",
        "assistant.channels.install": "Parcourir les applications disponibles",
        "assistant.channels.configure": "Configurer",
        "assistant.channels.addMore": "Ajouter d'autres applications",

        "assistant.sheet.close": "Fermer",
        "assistant.sheet.viewNotFound": "Cette vue est introuvable.",
        "assistant.sheet.loadError": "Impossible de charger la vue de configuration.",
        // Tool Call Display Mode
        "Verbose": "Verbose",
        "Light": "Light",
        "Quiet": "Quiet",
        "Affichage des appels d'outils": "Affichage des appels d'outils",
        "toolCallDisplayMode.verbose.description": "Complet : traces des outils dépliées et affichées",
        "toolCallDisplayMode.light.description": "Compact : noms des outils affichés mais traces repliées",
        "toolCallDisplayMode.quiet.description": "Masqué : aucun appel d'outil affiché dans les conversations",
      },
    },
  },
  lng: "fr", // default language
  fallbackLng: "en",
  interpolation: {
    escapeValue: false,
  },
});

export default i18n;
