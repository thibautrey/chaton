...
- `cloud-realtime` now keeps a bounded replay buffer per cloud instance and the desktop requests that replay immediately after websocket connect, so recent conversation events can be re-applied after a short disconnect.
