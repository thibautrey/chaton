...
Readiness probes are bounded, and simultaneous start requests for the same extension are coalesced. This keeps slow or unreachable local endpoints from making the app unresponsive and prevents duplicate child processes when startup is triggered from multiple UI/runtime paths at once.
