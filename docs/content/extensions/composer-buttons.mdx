...
interface ComposerContextUsageData {
  usedTokens: number;       // Total tokens consumed so far
...
The context usage circular progress ring is implemented as a built-in widget-mode composer button (`@chaton/context-usage`). It shows how much of the current model's context window has been consumed.
