...
interface ComposerButtonAction {
  ...
  icon: string;                        // Supported Lucide icon name (used in icon mode)
...
- **Use supported Lucide icons** for icon-mode buttons (`Mic`, `Camera`, `Zap`, `Filter`, `Search`, `Settings`, `Plus`, etc.). Unsupported names render as text so Chatons does not bundle the full icon set.
