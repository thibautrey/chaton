...
- `registerExtensionServerFromUi(...)` -- register a local server at runtime
- `requirementSheet.confirm()` -- confirm and close a requirement sheet
- `requirementSheet.dismiss()` -- dismiss a requirement sheet
- `requirementSheet.openSettings()` -- open Settings and close the sheet

The `requirementSheet` methods are available inside requirement sheet iframes. See [Requirement Sheets](/extensions/requirement-sheets) for details.
