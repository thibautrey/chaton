...
An LLM tool can return a requirement sheet to prompt the user for missing prerequisites (API keys, auth, settings). The sheet appears as an inline iframe at the top of the conversation.

To trigger a requirement sheet, return an error with a `requirementSheet` property:
...
The sheet iframe has access to `window.chaton.requirementSheet` with `confirm()`, `dismiss()`, and `openSettings()` methods.

For the full requirement sheet API and examples, see [Requirement Sheets](/extensions/requirement-sheets).
