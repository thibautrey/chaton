...
Keys are trimmed, must be non-empty, must not contain control characters, and are limited to 200 characters. Values must be JSON-serializable and are limited to 256 KiB after JSON serialization.
...
File contents must be UTF-8 strings and are limited to 5 MiB per read or write. Symlinked paths inside the extension data directory are rejected so file APIs stay confined to the extension storage root.
...
Payloads must be JSON-serializable and are limited to 256 KiB after JSON serialization. `idempotencyKey` is trimmed, must not contain control characters, and is limited to 160 characters.
...
Queue payloads must be JSON-serializable and are limited to 256 KiB after JSON serialization. Topics, consumer IDs, message IDs, dates, idempotency keys, and consume limits are validated before messages are persisted or claimed.
...
`limit` is optional. When provided, it must be an integer from 1 to 100.
