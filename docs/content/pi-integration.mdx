...
- Dev-only desktop automation can opt out of the normal single-instance app lock with `CHATON_ALLOW_AUTOMATION_INSTANCE=1` so a QA harness can launch a second real Electron window against the same development build. The same automation mode may set `CHATON_USER_DATA_DIR` to an isolated temporary profile for destructive or model-calling smoke tests; normal launches ignore that variable unless `CHATON_ALLOW_AUTOMATION_INSTANCE=1` is also present.
...
Where `<userData>` is `<appData>/Chatons` (overridden from Electron's default in `main.ts`). Automated QA may temporarily override this with `CHATON_USER_DATA_DIR`, but only while `CHATON_ALLOW_AUTOMATION_INSTANCE=1` is set.
...
ACP persistence remains durable per event, but renderer broadcasts are batched for rapid status churn. The ACP router should hydrate and send the latest full conversation state once per short batch instead of reloading SQLite state and broadcasting a whole side-panel snapshot for every individual task/status tick.
