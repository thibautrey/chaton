...
4. Override `userData` path to `<appData>/Chatons`; automated QA may override this with `CHATON_USER_DATA_DIR` only when `CHATON_ALLOW_AUTOMATION_INSTANCE=1` is also set
...
Real Electron smoke tests should use `CHATON_ALLOW_AUTOMATION_INSTANCE=1` when they need a second app instance. If a test needs to send prompts, mutate conversations, or exercise startup against copied state, it should also provide an isolated `CHATON_USER_DATA_DIR`; the main process deliberately ignores that variable outside automation mode to prevent accidental relocation of a user's production Chatons data.
...
- `electron/acp/router.ts` broadcasts `chaton:acp:event` so the renderer can update in real time. Rapid status churn should be coalesced into one latest full-state broadcast per short batch instead of one SQLite rehydrate and whole-state IPC payload per tick.
