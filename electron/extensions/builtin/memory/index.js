(function () {
  ...
  function buildShell() {
    ...
    titleGroup.appendChild(ui.el("h1", "ce-page-title", "Mémoire"));
    titleWrap.appendChild(titleGroup);
    titleWrap.appendChild(
      ui.el(
        "p",
        "ce-page-description",
        "Mémoire interne Chatons — stockage global et par projet, recherche sémantique locale.",
      ),
    );

    var toolbar = ui.el("div", "ce-toolbar");
    var refreshBtn = ui.createButton({
      text: "Rafraichir",
      variant: "outline",
    });
    ...
    query.id = "query";
    query.placeholder =
      "Rechercher un souvenir, une preference, une decision...";
    searchCard.body.appendChild(
      ui.createField({ label: "Requete", input: query }),
    );
    ...
    searchCard.body.appendChild(
      ui.createField({ label: "Scope", input: scope }),
    );
    ...
    searchCard.body.appendChild(
      ui.createField({
        label: "Projet",
        input: project,
        help: "Optionnel pour les recherches mixtes ou scope=project.",
      }),
    );
    ...
    createCard.body.appendChild(
      ui.el("h2", "ce-section-title", "Ajouter une memoire"),
    );
    ...
    createCard.body.appendChild(
      ui.createField({ label: "Projet", input: createProject }),
    );
  ...
  function renderEntries(entries) {
    ...
    entries.forEach(function (entry) {
      ...
      var dot = ui.el(
        "span",
        entry.scope === "global" ? "ce-dot ce-dot--success" : "ce-dot",
      );
      var content = ui.el("div", "ce-list-row__content");
      var title = ui.el(
        "p",
        "ce-list-row__title",
        (entry.title || "Sans titre") + " [" + entry.scope + "]",
      );
      var meta = ui.el(
        "p",
        "ce-list-row__meta",
        (entry.kind || "fact") + " · " + (entry.tags || []).join(", "),
      );
      ...
      row.appendChild(
        ui.el("div", "ce-list-row__aside", nowRel(entry.updatedAt)),
      );
