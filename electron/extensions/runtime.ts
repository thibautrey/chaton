...
import {
  chatonsGetHiddenProjects,
  chatonsGetProject,
  chatonsGetProjectConversations,
  chatonsGetVisibleProjects,
  chatonsListProjects,
...
import {
  ...
  getBuiltinAutomationExtensionId,
  getExtensionManifest,
  getExtensionRuntimeHealth as getRegistryRuntimeHealth,
  initializeExtensionsRuntime as initializeRegistry,
...
import {
  buildToolCatalogFromExposedTools,
  getToolCatalogEntry,
  searchToolCatalog,
...
const eventBuffer = new Map<
  string,
  { extensionId: string; key: string; items: unknown[] }
>();
...
async function appendEventToExtensionKv(
  extensionId: string,
  topic: string,
  payload: unknown,
) {
...
function getPiRuntimeManager(): PiSessionRuntimeManager {
  if (!piRuntimeManagerInstance) {
    ...
    const workspace =
      require("../ipc/workspace.js") as typeof import("../ipc/workspace.js");
    piRuntimeManagerInstance =
      workspace.piRuntimeManager as PiSessionRuntimeManager;
  }
      ...
      return Promise.resolve({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to initialize Pi executor",
      });
    }
  },
});
...
export function getExposedExtensionTools(): ExposedExtensionToolDefinition[] {
  return Array.from(runtimeState.manifests.keys()).flatMap((extensionId) =>
    buildExtensionToolDefinitions(extensionId, extensionsCall as never),
  );
}
...
export function subscribeExtension(
  ...
  if (!hasCapability(extensionId, "events.subscribe")) {
    return {
      ok: false,
      message: `Extension ${extensionId} missing capability events.subscribe`,
    };
  }
...
export function emitHostEvent(
  ...
  for (const subscription of runtimeState.subscriptions.values()) {
    ...
    if (
      subscription.options?.conversationId &&
    ...
    if (
      subscription.options?.projectId &&
      typeof payload === "object" &&
      payload &&
      !Array.isArray(payload)
    ) {
    ...
    void automationRuntime.runAutomationOnEvent(
...
export function extensionsCall(
    ...
    const inferredConversationId =
      ...
      context.conversationId.trim()
        ...
        : typeof context?.toolCallId === "string"
          ? getPiRuntimeManager().getConversationIdForToolCall(
              context.toolCallId,
            )
          : undefined;
    return (
      automationRuntime.extensionsCallAutomation(apiName, payload, {
        conversationId: inferredConversationId,
      }) ?? {
        ok: false,
        error: {
          code: "not_found",
          message: `API ${apiName} not found on ${extensionId}`,
        },
      }
    );
  }
  ...
  if (extensionId === BUILTIN_PROJECTS_ID) {
    if (apiName === "chatons_list_projects")
      return chatonsListProjects(payload);
    if (apiName === "chatons_get_project") return chatonsGetProject(payload);
    if (apiName === "chatons_get_project_conversations")
      return chatonsGetProjectConversations(payload);
    if (apiName === "chatons_get_hidden_projects")
      return chatonsGetHiddenProjects();
    if (apiName === "chatons_get_visible_projects")
      return chatonsGetVisibleProjects();
    if (apiName === "chatons_update_project_visibility")
      return chatonsUpdateProjectVisibility(payload);
    return {
      ok: false,
      error: {
        code: "not_found",
  ...
  if (extensionId === BUILTIN_EXTENSION_MANAGER_ID) {
    if (apiName === "extension.search_marketplace") {
      ...
      if (!catalog.ok) {
        return {
          ...
          error: {
            code: "internal",
            message: "Failed to load extension catalog",
          },
        };
      }
    ...
    if (apiName === "extension.install") {
      ...
      if (!id) {
        return {
          ok: false,
          error: { code: "invalid_args", message: "Extension ID is required" },
        };
      }
    ...
    if (apiName === "extension.uninstall") {
      ...
      return result.ok
        ? {
            ok: true,
            data: { message: "Extension uninstalled", id: result.id },
          }
        : {
            ok: false,
            error: {
              code: "internal",
              message: result.message || "Uninstall failed",
            },
          };
    }
    ...
    if (apiName === "extension.get_logs") {
      ...
      if (!result.ok) {
        return {
          ok: false,
          error: { code: "internal", message: "Failed to get logs" },
        };
      }
      ...
      return {
        ok: true,
        data: { id, content, lines: content.split("\n").length },
      };
    }
    ...
    if (apiName === "extension.check_updates") {
      ...
      return result.ok
        ...
        : {
            ok: false,
            error: { code: "internal", message: "Failed to check for updates" },
          };
    }
...
export function runExtensionsQueueWorkerCycle() {
  void automationRuntime.runExtensionsQueueWorkerCycle(
    queueConsume,
    queueAck,
    queueNack,
  );
  ...
  return {
    ...
    byExtension: listExtensionManifests().map((manifest) => ({
      ...
      capabilitiesUsed: Array.from(
        runtimeState.capabilityUsage.get(manifest.id) ?? new Set(),
      ),
    })),
  };
}
...
async function handleListProjects(
  ...
  const limit =
    typeof params?.limit === "number" && params.limit > 0 ? params.limit : null;
    ...
    return {
      ...
      data: {
        project: {
          ...
          location: project.location ?? "local",
          cloudInstanceId: project.cloud_instance_id ?? null,
          organizationId: project.organization_id ?? null,
          organizationName: project.organization_name ?? null,
          cloudStatus: project.cloud_status ?? null,
...
async function handleGetVisibleProjects(
  _payload: unknown,
): Promise<ExtensionHostCallResult> {
  try {
    const db = getDb();
