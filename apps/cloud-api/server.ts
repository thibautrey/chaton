...
async function waitForShutdownStep(
  ...
  const timeout = new Promise<void>((resolve) => {
    timeoutId = setTimeout(() => {
      console.warn(`[cloud-api] ${label} exceeded ${timeoutMs}ms during shutdown; continuing`)
      resolve()
    }, timeoutMs)
    timeoutId.unref?.()
  })
  ...
  try {
    await Promise.race([
      operation.catch((error) => {
        console.error(`[cloud-api] ${label} failed during shutdown`, error)
      }),
      timeout,
    ])
  } finally {
...
async function shutdown(signal: NodeJS.Signals): Promise<void> {
  ...
  shutdownPromise = (async () => {
    ...
    const forceCloseTimer = setTimeout(() => {
      console.warn('[cloud-api] forcing remaining HTTP connections closed during shutdown')
    ...
    await waitForShutdownStep('store close', store.close(), 5_000)
  })()
